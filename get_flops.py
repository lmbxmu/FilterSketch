    ...
    if is_conv:
        ...
        for idx, param in enumerate(model.parameters()):
            ...
            if hasattr(model,'last_prune_num'):
                ...
                total += (f) * (c-last_prune_num) * param.numel() / f / c
            else:
                total += (f) * param.numel() / f
        return total
    else:
...
def measure_layer(layer, x, print_name):
    ...
    if type_name in ['Conv2d']:
        ...
        #     delta_ops = (layer.in_channels-layer.last_prune_num) * (layer.out_channels) * layer.kernel_size[0] * \
        ...
        if print_name:
            ...
            #       'params:',delta_params, ' flops:',delta_ops_ori)
            print([layer.out_channels, layer.in_channels, layer.kernel_size[0], layer.kernel_size[1]],
                  'params:', delta_params, ' flops:', delta_ops_ori)
